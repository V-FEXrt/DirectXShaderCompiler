...
void main(uint ID : SV_GroupID) {
  ...
  // CHECK-SAME: i1 true, <8 x half> %[[VEC5]], <8 x half> %[[VEC6]])  ; LinAlgMatrixOuterProduct(isSigned,vectorA,vectorB)
  ...
  // CHECK-SAME: i32 -2147483619, i1 true, <8 x half> %[[VEC5]], <4 x half> %[[VEC20]]) ; LinAlgMatrixOuterProduct(isSigned,vectorA,vectorB)
  ...
  // CHECK-SAME: ; LinAlgMatrixOuterProduct(isSigned,vectorA,vectorB)
  ...
  // CHECK-SAME: i32 0, i32 64, i1 true, <4 x half> <half 0xH4926, half 0xH4926, half 0xH4926, half 0xH4926>)
  // CHECK-SAME: ; LinAlgVectorAccumulateToDescriptor(handle,offset,align,isSigned,vector)
  ...
  // CHECK-SAME: i32 8, i32 64, i1 true, <8 x half> %{{[0-9]+}}) ; LinAlgVectorAccumulateToDescriptor(handle,offset,align,isSigned,vector)
  InterlockedAccumulate(RWBAB, 8, vec2);

  // CHECK: call void @dx.op.linAlgVectorAccumulateToDescriptor.v4i32(i32 -2147483617, %dx.types.Handle %{{[0-9]+}},
  // CHECK-SAME: i32 16, i32 64, i1 false, <4 x i32> <i32 1, i32 2, i32 3, i32 4>)
  // CHECK-SAME: ; LinAlgVectorAccumulateToDescriptor(handle,offset,align,isSigned,vector)
  InterlockedAccumulate(RWBAB, 16, unsignedVec);
}
