...
void main() {
  ...
  // CHECK-SAME: (i32 -2147483636, i1 true, i32 {{.*}})  ; LinAlgFillMatrix(isSigned,value)
  ...
  // CHECK-SAME: (i32 -2147483636, i1 true, float {{.*}})  ; LinAlgFillMatrix(isSigned,value)
  ...
  __builtin_LinAlg_FillMatrix(mat2, true, 3.14);


  // CHECK: %{{.*}} = call %dx.types.LinAlgMatrixC5M8N4U0S2 @dx.op.linAlgFillMatrix.mC5M8N4U0S2.f64
  // CHECK-SAME: (i32 -2147483636, i1 true, double {{.*}})  ; LinAlgFillMatrix(isSigned,value)
  ...
  __builtin_LinAlg_FillMatrix(mat3, true, dVal);


  // CHECK: %{{.*}} = call %dx.types.LinAlgMatrixC5M4N4U1S1 @dx.op.linAlgFillMatrix.mC5M4N4U1S1.i64
  // CHECK-SAME: (i32 -2147483636, i1 true, i64 {{.*}})  ; LinAlgFillMatrix(isSigned,value)
  ...
  __builtin_LinAlg_FillMatrix(mat4, true, i64Val);
}
