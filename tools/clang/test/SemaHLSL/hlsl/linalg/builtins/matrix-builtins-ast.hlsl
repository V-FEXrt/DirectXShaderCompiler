...
// CHECK-NEXT: ParmVarDecl {{.*}} isSigned 'bool'
...
// CHECK: FunctionDecl {{.*}} implicit used __builtin_LinAlg_VectorAccumulateToDescriptor 'void (RWByteAddressBuffer, unsigned int, unsigned int, bool, vector<float, 4>)' extern
// CHECK-NEXT: ParmVarDecl {{.*}} buf 'RWByteAddressBuffer'
// CHECK-NEXT: ParmVarDecl {{.*}} offset 'unsigned int'
// CHECK-NEXT: ParmVarDecl {{.*}} align 'unsigned int'
// CHECK-NEXT: ParmVarDecl {{.*}} isSigned 'bool'
...
// CHECK-NEXT: AvailabilityAttr {{.*}} Implicit  6.10 0 0 ""
  __builtin_LinAlg_VectorAccumulateToDescriptor(Buf, 10, 64, true, input);
}
