...
// CHECK: FunctionDecl {{.*}} implicit used __builtin_LinAlg_MatrixVectorMultiplyAdd 'void (vector<float, 4> &, __builtin_LinAlg_Matrix, vector<float, 4>, unsigned int, vector<float, 4>, unsigned int)' extern
// CHECK-NEXT: ParmVarDecl {{.*}} ret 'vector<float, 4> &&__restrict'
// CHECK-NEXT: ParmVarDecl {{.*}} mat '__builtin_LinAlg_Matrix'
